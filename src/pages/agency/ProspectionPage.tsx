...
const ProspectionPage = () => {
  ...
  const handleContractFinalization = async () => {
    ...
    try {
      ...
      if (selectedReservation.type === 'Location') {
        locationUpsertData.rental_start_date = rentalStartDate ? new Date(rentalStartDate).toISOString() : null;
        locationUpsertData.rental_end_date = rentalEndDate ? new Date(rentalEndDate).toISOString() : null;
      }
      
      if (locationData) {
        console.log('Updating location with data:', locationUpsertData);
        ...
        console.log('Creating new location with data:', locationUpsertData);
